...
v1.1.0 introduces significant improvements.  See the change log here.
...
The *timeout* option is introduced in v1.1.0.  A timeout value, in minutes, will 
enable automatic retries to connect to the server when recoverable errors -
`ECONNREFUSED` and `EHOSTUNREACH` - are encountered.  During this timeout period, 
...
network sockets will be closed and a 'disconnected' event emitted.  If timeout is 
used, it is recommended to set its value to > 1 minute.  Also recommended is to use 
V68 of pigpio library.  
